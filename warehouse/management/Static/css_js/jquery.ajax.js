...
function RequestHandler(request,response,resdata){
	...
	if(request.isneedblock == true)
	{	$.blockUI({ message: '<i class="fa fa-delicious fa-pulse" style="font-size: 60px;color: #fff;"></i>' ,
css: { width: '30%', border:'0px solid #FFFFFF',cursor:'wait',background:'transparent'},
  overlayCSS:  { backgroundColor: 'rgba(0,0,0,0.7)',opacity:1,cursor:'wait'} 
	...
	var tempurl = domainname+request.url;
...
function SecToTimeFormat(seconds)
{
	...
	var timeformat = ((h > 0 ? (h < 10 ? "0" : "") + h + ":" : "00:") + (m > 0 ? (m < 10 ? "0" : "") + m + ":" : "00:") + (s < 10 ? "0" : "") + s);
	return timeformat;
}

function MinToTimeFormat(seconds)
{
	var d = Number(seconds);
	var h = Math.floor(d / 3600);
	var m = Math.floor(d % 3600 / 60);
	//var s = Math.floor(d % 3600 % 60);
	var timeformat = ((h > 0 ? (h < 10 ? "0" : "") + h + ":" : "00:") + (m > 0 ? (m < 10 ? "0" : "") + m + "" : "00") );
	return timeformat;
}
function SecToMinFormat(seconds)
{
	
	var d = Number(seconds);
	var m = Math.floor(d % 3600 / 60);
	var s = Math.floor(d % 3600 % 60);
	var timeformat = ((m > 0 ? (m < 10 ? "0" : "") + m + ":" : "00:") + (s < 10 ? "0" : "") + s);
	return timeformat;
}

function SecToHrMinFormat(seconds)
{
	var d = Number(seconds);
	var h = Math.floor(d / 3600);
	var m = Math.floor(d % 3600 / 60);
